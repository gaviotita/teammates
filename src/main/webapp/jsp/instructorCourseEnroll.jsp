    ...
    <div class="container" id="frameBodyWrapper">
       ...
       <div class="panel panel-primary">
            <div class="panel-body fill-plain">
                <form action="<%=data.getInstructorCourseEnrollSaveLink(data.courseId)%>" method="post" class="form form-horizontal"> 
                
                <input type="hidden" name="action" id="action" value="only"><% if(data.action != null) %><%=data.action %>
                    <div class="form-group">
                         <label class="col-sm-1 control-label">Student Name:</label>
                         <div class="col-sm-11">
                             <input class="form-control" name="namestudent" id="namestudent"><% if(data.nameStudent != null) %><%=data.nameStudent %>
                         </div>
                     </div>
                     <div class="form-group">
                         <label class="col-sm-1 control-label">Team Name:</label>
                         <div class="col-sm-11">
                             <input class="form-control" name="teamstudent" id="teamstudent"><% if(data.teamStudent != null) %><%=data.teamStudent %>
                         </div>
                     </div>
                     <div class="form-group">
                         <label class="col-sm-1 control-label">E-mail Address:</label>
                         <div class="col-sm-11">
                             <input class="form-control" name="emailstudent" id="emailstudent"><% if(data.emailStudent != null) %><%=data.emailStudent %>
                         </div>
                     </div>
                     <div class="form-group">
                         <label class="col-sm-1 control-label">Comments:</label>
                         <div class="col-sm-11">
                             <textarea class="form-control" rows="6" name="commentstudent" id="commentstudent"><% if(data.commentStudent != null) %><%=data.commentStudent %></textarea>
                         </div>
                     </div> 
                    <br>
                    <jsp:include page="<%=Const.ViewURIs.STATUS_MESSAGE%>" />
                        <button type="submit" title="addStudent" id="button_add" name="button_add" class="btn btn-primary btn-md">
                        Add Student
                        </button>
                </form>
            </div>
        ...
        <%-- Agregar estudiantes en por plantilla --%>
        <div id="headerOperation">
            <h1>Add a Students Group</h1>
        </div>
        ...
        <div class="panel panel-primary">
          <div class="panel-body fill-plain">
            ...
            <form action="<%=data.getInstructorCourseEnrollSaveLink(data.courseId)%>" method="post" class="form-horizontal" role="form">
              ...
              <input type="hidden" name="action" id="action" value="enroll"><% if(data.action != null) %><%=data.action %>
              <div class="col-md-12">
                <div class="form-group">
